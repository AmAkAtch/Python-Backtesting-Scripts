...
public class ChunkErosionMap {
    ...
    /**
     * Removes entries that haven't been touched in over {@code timeoutTicks}, skipping any
     * entry whose tracked block matches {@code exempt} (used to protect already-placed eroded
     * blocks, which manage their own lifecycle via block-level de-erosion timeouts and must
     * always keep a live cooldown entry).
     *
     * @return the number of entries removed.
     */
    public int pruneStale(long currentGameTime, long timeoutTicks, Predicate<Block> exempt) {
        int before = entries.size();
        entries.entrySet().removeIf(e -> {
            ErosionEntry entry = e.getValue();
            if (exempt.test(entry.getTrackedBlock())) return false;
            return currentGameTime - entry.getLastTouchedGameTime() > timeoutTicks;
        });
